...
def _latest_row(versions: list[RoleTemplateVersion]) -> RoleTemplateVersion | None:
    ...
    try:
        ...
        return versions[0]  # relationship is ordered by created_at DESC
...
async def list_roles(
    ...
    stmt = select(RoleTemplate).options(selectinload(RoleTemplate.versions)).order_by(RoleTemplate.slug)
...
async def create_role(
    ...
    role = await session.scalar(
        select(RoleTemplate)
        .options(selectinload(RoleTemplate.versions))
        .where(RoleTemplate.slug == body.slug)
    )
    if role is None:
        role = RoleTemplate(
            slug=body.slug,
